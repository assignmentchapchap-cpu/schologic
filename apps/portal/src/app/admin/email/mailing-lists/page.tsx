...
export default function MailingListsPage() {
    ...
    // View mode: 'contacts' | 'prospects'
    const [activeTab, setActiveTab] = useState<'contacts' | 'prospects'>('prospects');
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-indigo-500/20">
                        ...
                        {loading ? (
                            ...
                            <div className="divide-y divide-slate-100">
                                {audiences.map(audience => (
                                    <div
                                        key={audience.id}
                                        className={`group flex items-center justify-between p-4 cursor-pointer transition-all duration-200 border-b border-slate-50 last:border-0 ${selectedAudience?.id === audience.id
                                                ? 'bg-gradient-to-r from-indigo-50/80 to-transparent border-l-4 border-l-indigo-500 shadow-sm relative overflow-hidden'
                                                : 'hover:bg-slate-50 border-l-4 border-l-transparent text-slate-600 hover:text-slate-900'
                                            }`}
                    ...
                    {selectedAudience ? (
                        <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                                            <button
                                                ...
                                                className={`px-3 py-1 text-xs font-bold rounded-md transition-shadow ${activeTab === 'prospects' ? 'bg-white shadow-sm text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}
                                            >
                                                Prospects
                                            </button>
                                            <button
                                                onClick={() => setActiveTab('contacts')}
                                                className={`px-3 py-1 text-xs font-bold rounded-md transition-shadow ${activeTab === 'contacts' ? 'bg-white shadow-sm text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}
                                            >
                                                Subscribers
                                            </button>
                                        </div>
                                    </div>
                                    <p className="text-xs text-slate-400 mt-1">{activeTab === 'contacts' ? `${contacts.length} synced subscribers` : 'Outbound outreach targets'}</p>
                                </div>
                                {activeTab === 'contacts' && (
                                    <button
                            ...
                            {activeTab === 'contacts' ? (
                                ...
                                <ProspectsPanel listId={selectedAudience.id} listName={selectedAudience.name} />
                            )}
                        </div>
                    ) : (
